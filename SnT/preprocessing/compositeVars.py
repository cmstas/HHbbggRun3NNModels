...
def compositeVariables(input_variables, df_columns, df=None):
    ...
    variable_definitions = {
        "boosted": {
            ...
            "calculation": lambda df: (
                (
                    df[["fatjet1_pt", "fatjet2_pt", "fatjet3_pt", "fatjet4_pt"]] > 600
                ).any(axis=1)
            ).astype(int),
        },
        "VBF": {
        ...
        "nonRes_lead_bjet_massOverMjj_PNet_all": {
            "dependencies": [
                "nonRes_lead_bjet_mass_PNet_all",
                "nonRes_dijet_mass_PNet_all",
            ],
            "calculation": lambda df: df["nonRes_lead_bjet_mass_PNet_all"]
            / df["nonRes_dijet_mass_PNet_all"],
        },
        "nonRes_lead_bjet_ptOverMjj_PNet_all": {
            "dependencies": [
                "nonRes_lead_bjet_pt_PNet_all",
                "nonRes_dijet_mass_PNet_all",
            ],
            "calculation": lambda df: df["nonRes_lead_bjet_pt_PNet_all"]
            / df["nonRes_dijet_mass_PNet_all"],
        },
        "nonRes_sublead_bjet_massOverMjj_PNet_all": {
            "dependencies": [
                "nonRes_sublead_bjet_mass_PNet_all",
                "nonRes_dijet_mass_PNet_all",
            ],
            "calculation": lambda df: df["nonRes_sublead_bjet_mass_PNet_all"]
            / df["nonRes_dijet_mass_PNet_all"],
        },
        "nonRes_sublead_bjet_ptOverMjj_PNet_all": {
            "dependencies": [
                "nonRes_sublead_bjet_pt_PNet_all",
                "nonRes_dijet_mass_PNet_all",
            ],
            "calculation": lambda df: df["nonRes_sublead_bjet_pt_PNet_all"]
            / df["nonRes_dijet_mass_PNet_all"],
        },
        "nonRes_dijet_PtOverMggjj_PNet_all": {
            "dependencies": ["nonRes_dijet_pt_PNet_all", "nonRes_HHbbggCandidate_mass"],
            "calculation": lambda df: df["nonRes_dijet_pt_PNet_all"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "nonRes_dijet_massOverMggjj_PNet_all": {
            ...
            "calculation": lambda df: df["nonRes_dijet_mass_PNet_all"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "lead_bjet_massOverMjj": {
            "dependencies": ["nonRes_lead_bjet_mass", "nonRes_dijet_mass"],
            "calculation": lambda df: df["nonRes_lead_bjet_mass"]
            / df["nonRes_dijet_mass"],
        },
        "sublead_bjet_massOverMjj": {
            "dependencies": ["nonRes_sublead_bjet_mass", "nonRes_dijet_mass"],
            "calculation": lambda df: df["nonRes_sublead_bjet_mass"]
            / df["nonRes_dijet_mass"],
        },
        "dipho_PtOverMggjj": {
        ...
        "dijet_PtOverMggjj": {
            ...
            "calculation": lambda df: df["nonRes_dijet_pt"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "dijet_PtOverMjj": {
        ...
        "dijet_massOverMggjj": {
            ...
            "calculation": lambda df: df["nonRes_dijet_mass"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "deltaPhi_g1_g2": {
            "dependencies": ["sublead_phi", "lead_phi"],
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["sublead_phi"], row["lead_phi"]), axis=1
            ),
        },
        "deltaPhi_g1_j1": {
            "dependencies": ["nonRes_lead_bjet_phi", "lead_phi"],
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["nonRes_lead_bjet_phi"], row["lead_phi"]),
                axis=1,
            ),
        },
        "deltaPhi_g1_j2": {
            ...
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["nonRes_sublead_bjet_phi"], row["lead_phi"]),
                axis=1,
            ),
        },
        "lead_eta_mirrored": {
        ...
        "lead_bjet_eta_mirrored": {
            ...
            "calculation": lambda df: df["nonRes_lead_bjet_eta"]
            * np.sign(df["lead_eta"]),
        },
        "sublead_bjet_eta_mirrored": {
            "dependencies": ["nonRes_sublead_bjet_eta", "lead_eta"],
            "calculation": lambda df: df["nonRes_sublead_bjet_eta"]
            * np.sign(df["lead_eta"]),
        },
        "deltaR_photons": {
        ...
        "deltaR_jets": {
            ...
            "calculation": lambda df: df.apply(
                lambda row: deltaR(
                    row["nonRes_lead_bjet_eta"],
                    row["nonRes_lead_bjet_phi"],
                    row["nonRes_sublead_bjet_eta"],
                    row["nonRes_sublead_bjet_phi"],
                ),
                axis=1,
            ),
        },
        "deltaEta_jets": {
            ...
            "calculation": lambda df: abs(
                df["nonRes_lead_bjet_eta"] - df["nonRes_sublead_bjet_eta"]
            ),
        },
        "deltaR_gg_jj": {
            ...
            "calculation": lambda df: df.apply(
                lambda row: deltaR(
                    row["eta"],
                    row["phi"],
                    row["nonRes_dijet_eta"],
                    row["nonRes_dijet_phi"],
                ),
                axis=1,
            ),
        },
        ...
        "deltaPhi_gg_jj": {
            ...
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["phi"], row["nonRes_dijet_phi"]), axis=1
            ),
        },
        "deltaPhi_g1_jj": {
            "dependencies": ["lead_phi", "nonRes_dijet_phi"],
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["lead_phi"], row["nonRes_dijet_phi"]), axis=1
            ),
        },
        "deltaPhi_g2_jj": {
            "dependencies": ["sublead_phi", "nonRes_dijet_phi"],
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["sublead_phi"], row["nonRes_dijet_phi"]),
                axis=1,
            ),
        },
        "deltaPhi_j1_gg": {
            ...
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["nonRes_lead_bjet_phi"], row["phi"]), axis=1
            ),
        },
        "deltaPhi_j2_gg": {
            "dependencies": ["nonRes_sublead_bjet_phi", "phi"],
            "calculation": lambda df: df.apply(
                lambda row: deltaPhi(row["nonRes_sublead_bjet_phi"], row["phi"]), axis=1
            ),
        },
        "Max_mvaID": {
            "dependencies": ["lead_mvaID", "sublead_mvaID"],
            "calculation": lambda df: np.max(
                [df["lead_mvaID"], df["sublead_mvaID"]], axis=0
            ),
        },
        "Min_mvaID": {
            "dependencies": ["lead_mvaID", "sublead_mvaID"],
            "calculation": lambda df: np.min(
                [df["lead_mvaID"], df["sublead_mvaID"]], axis=0
            ),
        },
        "HHbbggCandidate_ptoverMggjj": {
            ...
            "calculation": lambda df: df["nonRes_HHbbggCandidate_pt"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "HHbbggCandidate_mass_mXv1": {
            ...
            "calculation": lambda df: df["nonRes_HHbbggCandidate_mass"]
            - df["nonRes_dijet_mass_PNet_all"]
            - df["mass"]
            + 2 * common.HIGGS_MASS,
        },
        ...
        "puppiMET_sumEtoverMggjj": {
            ...
            "calculation": lambda df: df["puppiMET_sumEt"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "puppiMET_ptoverMggjj": {
            "dependencies": ["puppiMET_pt", "nonRes_HHbbggCandidate_mass"],
            "calculation": lambda df: df["puppiMET_pt"]
            / df["nonRes_HHbbggCandidate_mass"],
        },
        "lead_bjet_btagTight": {
            "dependencies": ["nonRes_lead_bjet_btagPNetB"],
            "calculation": lambda df: (
                df["nonRes_lead_bjet_btagPNetB"] > 0.6915
            ).astype(int),
        },
        "sublead_bjet_btagTight": {
            "dependencies": ["nonRes_sublead_bjet_btagPNetB"],
            "calculation": lambda df: (
                df["nonRes_sublead_bjet_btagPNetB"] > 0.6915
            ).astype(int),
        },
        "lead_bjet_btagMedium": {
            "dependencies": ["nonRes_lead_bjet_btagPNetB"],
            "calculation": lambda df: (df["nonRes_lead_bjet_btagPNetB"] > 0.26).astype(
                int
            ),
        },
        "sublead_bjet_btagMedium": {
            "dependencies": ["nonRes_sublead_bjet_btagPNetB"],
            "calculation": lambda df: (
                df["nonRes_sublead_bjet_btagPNetB"] > 0.26
            ).astype(int),
        },
        "lead_bjet_btagLoose": {
            "dependencies": ["nonRes_lead_bjet_btagPNetB"],
            "calculation": lambda df: (
                df["nonRes_lead_bjet_btagPNetB"] > 0.0499
            ).astype(int),
        },
        "sublead_bjet_btagLoose": {
            "dependencies": ["nonRes_sublead_bjet_btagPNetB"],
            "calculation": lambda df: (
                df["nonRes_sublead_bjet_btagPNetB"] > 0.0499
            ).astype(int),
        },
        "lead_MVAID_WP80": {
            "dependencies": ["lead_isScEtaEB", "lead_mvaID", "lead_isScEtaEE"],
            "calculation": lambda df: (
                ((df["lead_isScEtaEB"] == True) & (df["lead_mvaID"] > 0.420473))
                | ((df["lead_isScEtaEE"] == True) & (df["lead_mvaID"] > 0.203451))
            ).astype(int),
        },
        "sublead_MVAID_WP80": {
            "dependencies": ["sublead_isScEtaEB", "sublead_mvaID", "sublead_isScEtaEE"],
            "calculation": lambda df: (
                ((df["sublead_isScEtaEB"] == True) & (df["sublead_mvaID"] > 0.420473))
                | ((df["sublead_isScEtaEE"] == True) & (df["sublead_mvaID"] > 0.203451))
            ).astype(int),
        },
        "lead_MVAID_WP90": {
            "dependencies": ["lead_isScEtaEB", "lead_mvaID", "lead_isScEtaEE"],
            "calculation": lambda df: (
                ((df["lead_isScEtaEB"] == True) & (df["lead_mvaID"] > 0.0439603))
                | ((df["lead_isScEtaEE"] == True) & (df["lead_mvaID"] > -0.249526))
            ).astype(int),
        },
        "sublead_MVAID_WP90": {
            "dependencies": ["sublead_isScEtaEB", "sublead_mvaID", "sublead_isScEtaEE"],
            "calculation": lambda df: (
                ((df["sublead_isScEtaEB"] == True) & (df["sublead_mvaID"] > 0.0439603))
                | (
                    (df["sublead_isScEtaEE"] == True)
                    & (df["sublead_mvaID"] > -0.249526)
                )
            ).astype(int),
        },
        # NW variables
        ...
        "lead_j_pT_OverHbbcand_mass": {
            ...
            "calculation": lambda df: df["nonRes_lead_bjet_pt"]
            / df["nonRes_dijet_mass"],
        },
        "phosublead_PtOverM": {
        ...
        "sublead_j_pT_OverHbbcand_mass": {
            ...
            "calculation": lambda df: df["nonRes_sublead_bjet_pt"]
            / df["nonRes_dijet_mass"],
        },
        "lead_photon_EnergyErrOverE": {
            "dependencies": ["lead_energyErr", "lead_energyRaw"],
            "calculation": lambda df: df["lead_energyErr"] / df["lead_energyRaw"],
        },
        "sublead_photon_EnergyErrOverE": {
            "dependencies": ["sublead_energyErr", "sublead_energyRaw"],
            "calculation": lambda df: df["sublead_energyErr"] / df["sublead_energyRaw"],
        },
    }
    ...
    for var in input_variables:
        ...
        if var in variable_definitions:
            ...
            print(
                f"Variable {var} not recognized, please add it in compositeVariables."
            )
